...
def extract_text_tesseract(image_array):
    ...
    try:
        ...
        return "", 0
    """Extract text using Tesseract OCR with NO preprocessing - raw approach"""
    try:
        # Save image to temporary file AS-IS
        with tempfile.NamedTemporaryFile(suffix='.png', delete=False) as temp_file:
            cv2.imwrite(temp_file.name, image_array)
            temp_path = temp_file.name
        
        try:
            # Try ONLY the most basic PSM modes - no complex configurations
            psm_modes = [3, 6]  # Automatic page segmentation, uniform text block
            best_result = ""
            best_confidence = 0
            
            for psm in psm_modes:
                # MINIMAL command - no character whitelists that might filter out text
                cmd = [
                    'tesseract', temp_path, 'stdout',
                    '--psm', str(psm)
                ]
